import { dimmablePlugInUnit, MatterbridgeEndpoint, powerSource } from 'matterbridge';
...
export const dimmableOutletDescriptor: DeviceDescriptor = {
  ...
  async create(ctx: DeviceContext, cfg: MqttDeviceConfig): Promise<void> {
    ...
    const ep = new MatterbridgeEndpoint([dimmablePlugInUnit, powerSource]);
    ...
    if (cfg.topicMoveToLevel) {
      ...
      ctx.onCmd(ep, 'moveToLevel', ((data: LevelRequest) => {
        ...
        ctx.publish(moveTopic, String(mqttBrightness), cfg.retain);
      }) as AnyHandler);
    }
