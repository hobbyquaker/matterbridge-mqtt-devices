import { basicVideoPlayer, MatterbridgeEndpoint, powerSource } from 'matterbridge';
...
export const basicVideoPlayerDescriptor: DeviceDescriptor = {
  type: 'basic-video-player',
  editableKeys: {
  ...
  async create(ctx: DeviceContext, cfg: MqttDeviceConfig): Promise<void> {
    ...
    const ep = new MatterbridgeEndpoint([basicVideoPlayer, powerSource]);
    ctx.initEp(ep, cfg, 0x8034);
    ...
    ctx.log.info(`✓ basic video player "${cfg.name}"`);
  },
};
