import { LaundryDryer } from 'matterbridge/devices';
...
export const laundryDryerDescriptor: DeviceDescriptor = {
  type: 'laundry-dryer',
  editableKeys: {
    publish: ['topicSetOnOff', 'topicSetOperationalState', 'topicSetWasherMode'],
    subscribe: [
      ...
      // dryer mode (reuses washer mode cluster)
      'topicWasherMode',
  ...
  applyDefaults(cfg, baseTopic) {
    return {
      ...
      topicDrynessLevel: cfg.topicDrynessLevel ?? `${baseTopic}/dryness`,
  ...
  async create(ctx: DeviceContext, cfg: MqttDeviceConfig): Promise<void> {
    ...
    // LaundryDryer single-class creates: OnOff (DeadFront), LaundryWasherMode,
    // LaundryDryerControls, TemperatureControl (level-based), OperationalState,
    // PowerSource (wired), Identify, BasicInfo
    const ep = new LaundryDryer(cfg.name, serial);
    ...
    // ── MQTT → Matter: dryer mode (LaundryWasherMode cluster) ───────────────
    ...
    // ── MQTT → Matter: dryer controls ───────────────────────────────────────

    if (cfg.topicDrynessLevel) {
      ctx.subscribe(cfg.topicDrynessLevel, (p) => {
        const val = ctx.parseFloatPayload(p, ['selectedDrynessLevel', 'dryness', 'value'], cfg.payloadDrynessLevelJsonPath);
        if (val !== null && !isNaN(val)) {
          // 0=Low, 1=Normal, 2=Extra, 3=Max
          ctx.setAttr(ep, CID.LaundryDryerControls, 'selectedDrynessLevel', Math.round(val));
        }
      });
    }
    ...
    ctx.log.info(`✓ laundry dryer "${cfg.name}"`);
  },
};
