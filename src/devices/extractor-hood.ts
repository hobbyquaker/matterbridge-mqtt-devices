import { extractorHood, MatterbridgeEndpoint } from 'matterbridge';

import type { DeviceContext, DeviceDescriptor, MqttDeviceConfig } from './types.js';
import { CID, COMMON_SETTINGS_KEYS, COMMON_SUBSCRIBE_KEYS } from './types.js';

export const extractorHoodDescriptor: DeviceDescriptor = {
  type: 'extractor-hood',
  editableKeys: {
  ...
  applyDefaults(cfg, baseTopic) {
    return {
      ...
      topicSetFanMode: cfg.topicSetFanMode ?? `${baseTopic}/fan-mode/set`,
    };
  },
  async create(ctx: DeviceContext, cfg: MqttDeviceConfig): Promise<void> {
    ...
    const ep = new MatterbridgeEndpoint([extractorHood]);
    ctx.initEp(ep, cfg, 0x802e);
    ...
    ctx.log.info(`✓ extractor hood "${cfg.name}"`);
  },
};
